...
|--------|--------------|-------------------|
| `bot.incolumitas.com` | Structured JSON scoreboard: intoli, fpscanner, webdriver | `intoli` 6/6 OK, `fpscanner` 14/15 OK, only `WEBDRIVER: FAIL` |
| `pixelscan.net` | WebRTC + canvas + font fingerprint surface | Green on most checks |
| `kasadatech.com` | Kasada bot detection | 🟡 May block even with tier-2 |
| `geetest.com/en/demo` | 极验 GeeTest behavior analysis | 🟡 May need human-in-the-loop |
| `datadome.co` | DataDome behavioral analysis | 🟡 Often needs persistent cookies |

## Reference detail

- `references/test-targets.md` — canonical detection sites with expected baseline behavior (what passing/failing each one tells you about the stack), supplementary targets, and a recorded baseline run.
