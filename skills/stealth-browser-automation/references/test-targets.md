...
The probe script (`scripts/probe.py`) runs these four canonical targets:
...
## Supplementary targets (not in probe, use for deeper diagnosis)

| Target | What it tests | Expected (tier-1) | Notes |
|--------|--------------|-------------------|-------|
| `https://bot.incolumitas.com/` | Structured JSON-format scoreboard | `intoli` 6/6 OK, `fpscanner` 14/15 OK, only `WEBDRIVER: FAIL` | The W3C spec `webdriver` flag can't be removed without breaking the API surface. Anything more than one fail = patches degraded. |
| `https://pixelscan.net/` | WebRTC + canvas + font fingerprint surface | Green on most checks | Good for spot-checking WebRTC leak prevention and font fingerprinting after `geoip=True`. |
| `https://www.kasadatech.com/` | Kasada bot detection | 🟡 May block even with tier-2 | Kasada is a different class of protection. Headed + residential is necessary but not always sufficient. |
| `https://www.geetest.com/en/demo` | 极验 GeeTest behavior analysis | 🟡 May require human-in-the-loop | GeeTest tracks mouse trajectory and timing. `humanize=True` helps but may not be enough. |
| `https://datadome.co/` | DataDome behavioral analysis | 🟡 May require persistent cookies | Often needs `launch_persistent_context()` with a warm session. Fresh run = high chance of block. |

## Recorded baseline (CloakBrowser 0.4.7, Chromium 146.0.7680.177.5, Linux Debian 13)
...
- **bot.incolumitas.com shows more than `WEBDRIVER: FAIL`** → patches degraded, reinstall.
- **Supplementary targets (Kasada, GeeTest, DataDome) fail** → expected on tier-1. These are a different class of protection. Even tier-2 may not be enough; plan for solver services or human-in-the-loop.
